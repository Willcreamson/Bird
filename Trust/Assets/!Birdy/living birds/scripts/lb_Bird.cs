...
public class lb_Bird : MonoBehaviour {
	...
	void OnGroundBehaviors() {
		...
		if (idle) {
			...
			if (Random.value < Time.deltaTime * .33) {
				...
				anim.SetFloat("IdleAgitated", UnityEngine.Random.value);
			}
			//birds should fly to a new target about every 10 seconds
			if (UnityEngine.Random.value < Time.deltaTime * .1) {
				FlyAway();
			}
		}
	}
	...
	void Flee() {
		if (!dead) {
			...
			farAwayTarget += new Vector3(UnityEngine.Random.Range(-100, 100) * controller.birdScale, 10 * controller.birdScale, Random.Range(-100, 100) * controller.birdScale);
	...
	{
		public static TrashTags GetTrashTags(GarbageColor color)
		{
			switch (color)
			{
				case GarbageColor.Yellow:
					return new TrashTags { tag = "yellow", goodMessage = "Super travail ! En triant tes déchets, tu adoptes des réflexes bons pour la planète !", errorMessage = "Mince ! Tu t’es trompé de poubelle, réessaye ! " };
				case GarbageColor.Green:
					return new TrashTags { tag = "green", goodMessage = "Fantastique ! En plaçant ce verre dans la poubelle appropriée, tu as empêché qu'il ne se retrouve dans la nature pendant des centaines voire des milliers d'années. Tu as vraiment l'œil pour sauver notre planète !", errorMessage = "Mince ! Tu t’es trompé de poubelle, réessaye ! " };
				case GarbageColor.Red:
					return new TrashTags { tag = "red", goodMessage = "", errorMessage = "Mince ! Tu t’es trompé de poubelle, réessaye ! " };
				case GarbageColor.Blue:
					return new TrashTags { tag = "blue", goodMessage = "Bravo ! En mettant ce plastique dans la poubelle bleue, tu as évité qu'il ne pollue notre environnement pendant des centaines d'années. Tu es un super protecteur de la planète !", errorMessage = "Attention ! Il me semble que ce déchet trouverait mieux sa place dans une autre poubelle ! " };
				default:
    ...
    void displayFeedBack(GarbageColor trashColor, GarbageColor garbageColor)
    {
        string feedBack = "";
        if (trashColor != garbageColor)
        {
            TrashTags garbageTags = CodeColor.GetTrashTags(garbageColor);

            // Now you can switch on the color
            switch (garbageColor)
            {
                case GarbageColor.Yellow:
                    // Handle Yellow case
                    feedBack = garbageTags.goodMessage;
                ...
                case GarbageColor.Red:
                    // Handle Red case
                    feedBack = garbageTags.errorMessage;
                    ShowFeedback(feedBack);
                    break;
                case GarbageColor.Blue:
                    // Handle Blue case
        ...
        // Use the 'feedBack' variable as needed
    }
    ...
    void ShowFeedback(string message)
    {
        ...
        Invoke("CloseFeedbackWindow", 10f);
    }
